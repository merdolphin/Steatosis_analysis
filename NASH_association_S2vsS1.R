...
load(file = "/home/lina/scratch/18.NASH/mRNA.wilcox.AD0.05.rda")
load(file = "/home/lina/scratch/18.NASH/liver.wilcox.AD0.05.rda")


mRNA_count.S %>% dplyr::select(all_of(mRNA.wilcox.AD0.05$key)) -> mRNA.DA
liver_meta.S %>% dplyr::select(all_of(liver.wilcox.AD0.05$key)) -> liver.DA


cor(mRNA.DA, liver.DA, method="spearman") %>% melt() %>% 
  setNames(c("mRNA", "liver", "value")) -> mRNA.liver.sperman.cor
...
liver.AD.dict %>% dplyr::select(accession..HMDB.)
...
mRNA.liver.sperman.cor %>% filter(abs(value)>0.5) %>% mutate(liver=str_replace(liver, "V","")) %>%
  mutate(mRNA=str_replace(mRNA,"V","")) %>% left_join(mRNA.AD.dict, by=c("mRNA"="key")) %>%
  left_join(liver.AD.dict, by=c("liver" = "key")) %>% dplyr::select(accession..HMDB.) %>% distinct() 
...
AD.enrichedPathway.super.class
